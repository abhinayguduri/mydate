...
switch (url) {
	...
	case "popular":
		if(plugins['populars']['viewOnlyPremium'] == 'Yes'){
			meetPremiumNotification(1);					
		}
		updateSeo('populars');
	break;
...
function profileLinks(){
	...
	if(url == 'profile'){
		...
		var offset = $('.js-profile-languages-container').offset();
		var profileHeight = offset.top - 90;
...
function fastMessage(r_id){
	...
	var message = user_info.id+','+r_id+','+messageVal+',text,fast';
	var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',text';      
...
$("#upload-area").dmUploader({
    ...
    if(upType == 1){  	
		...
		$.ajax({
		  ...
		  success: function(response) {
			...
			$("#checkbox"+id).attr("onclick","manageUserMedia("+response.data.id+",2)");
    ...
    if(upType == 9){ //chat photo
		...
		var message = user_info.id+','+r_id+','+messageVal+','+sendType;  
		var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+','+sendType;
...
function sendCreditNow(){
	...
	if (user_info.credits < a) {
	 ...
	 swal({
		...
		var messageVal = '';
		var message = user_info.id+','+profile_info.id+','+messageVal+',credits'+','+a;
		var send = user_info.id+','+profile_info.id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',credits,'+a;      
...
function withdrawNow(){
	...
	if (user_info.payout < plugins['withdrawal']['minRequired']) {
	 ...
	 swal({
	    ...
	    text: site_lang[591]['text'] + ' ' + t + " " + plugins['settings']['currency'] + " " + site_lang[592]['text'] + " " + p,
...
$('#add-photo-spotlight').submit(function(e) {
	...
	$.get( aUrl, {action: 'addToSpotlight', query: uid});
...
function chatMessage(){
	...
	$('#c-send').submit(function(e) {
		...
		//send message
		var message = user_info.id+','+r_id+','+messageVal+',text';
		var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',text';      
...
function game_start(val=0){
	...
	if(game_array.length > 0){
			if(mobile == true){
				...
				$("#dis_name").html('<a href="#" onClick="goToProfile('+game_array[0].id+')"><b>'+ game_array[0].name +'</b>, '+ game_array[0].age +'</a>');
		...
		$.ajax({
			...
			success:  function (response) {	
				...
				$("#dis_name").html('<a href="#" onClick="goToProfile('+game_array[0].id+')"><b>'+ game_array[0].name +'</b>, '+ game_array[0].age +'<div id="dcheck"></div></a>');
				$("#dis_report").click(function(){ reportUser(game_array[0].id,'"'+game_array[0].name+'"','"'+game_array[0].photo+'"'); });
				$("#dis_block").click(function(){ reportUser(game_array[0].id,'"'+game_array[0].name+'"','"'+game_array[0].photo+'"'); });	
				$("#dis_chat").click(function(){ goToChat(game_array[0].id); });	
				}
...
$('#send-gift-form').submit(function(e) {
	...
	var r_id = $("#r_id").val();
	var messageVal = gift;
	var message = user_info.id+','+r_id+','+messageVal+',gift,'+gift_price;  
	var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',gift';      
	$('#send-gift').hide();
	$('#showGiftModal').hide();
	$.get( gUrl, {action: 'message', query: send} );		   	
	$.get( aUrl, {action: 'sendMessage', query: message} );
...
function peerConnect(con) {
	...
	peer = new Peer({host: ''+plugins['videocall']['videocallServer']+'', secure:true, port:443, key: 'peerjs'});
					...
					if(plugins['videocall']['creditsPerMinEnabled'] == "Yes"){
						swal({
							...
							if (!isConfirm){
								...
								user_info.credits = user_info.credits - plugins['videocall']['creditsPerMin'];
								$('.userCredits').text(user_info.credits);
...
function finishCallRT(r_id,duration){
	...
	var messageVal = '<span style="font-size:13px;"><strong>Videochat ended</strong> - duration '+duration+'</span>';

	var message = user_info.id+','+r_id+','+messageVal+',videocall';
	$.get( aUrl, {action: 'sendMessage', query: message} );	

	var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',videocall';      
	$.get( gUrl, {action: 'message', query: send} );		
...
//interaction('Xohan','Xohan','http://www.localhost/belloo/assets/sources/uploads/5cbab8016deed_reloco.jpeg','just sent you a message, are you going to see it?','message');

/*
setTimeout(function(){
interaction('Xohan','Xohan','http://www.localhost/belloo/assets/sources/uploads/5cbab8016deed_reloco.jpeg',site_lang[657]['text'],'visit');
},1200)
setTimeout(function(){
interaction('Xohan','Xohan','http://www.localhost/belloo/assets/sources/uploads/5cbab8016deed_reloco.jpeg','just gave you a like, what are you going to do about it?','like');
},2200)
*/
...
function sendGif(gif){
	...
	var r_id = $("#r_id").val();
	var messageVal = gif;
	var message = user_info.id+','+r_id+','+messageVal+',gif';  
	var send = user_info.id+','+r_id+','+user_info.profile_photo+','+user_info.first_name+','+messageVal+',gif';      
...
function purchasePremium(el){
	var price = $(el).attr('data-premium-price');
...
channel.bind(themeUpdate,function(data){
	if(user_info.admin == 1){
		if(data['reload'] == 1){
			...
			$.ajax({
				url: request_source()+'/belloo.php', 
				data: {
					action:"themeStylesReload",
					preset:data['preset'],
					theme:data['theme']						
				},	
				type: "post",			
				success: function(response) {
					$('#themeData').html(response);							
				},
			});
		}
	}	
...
function showProfileBio(el){
	...
	if(edit == 0) {
		...
		var query = user_info.id+','+bio+','+bioUrlQuery;
...
function updateProfileQuestionAnswer(el){
	...
	var query = user_info.id+','+q+','+answer;
